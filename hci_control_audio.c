...
typedef struct
{
    ...
    wiced_bt_dev_power_mgmt_status_t pm_status;
} avdt_conn_t;
...
typedef struct
{
    ...
    avdt_conn_t         avdt_conn[AVDT_CONN_MAX];
    avdt_conn_t         *avdt_conn_in_progress;  /* for tracking connection */
 ...
 ******************************************************************************/
static wiced_bool_t   av_app_next_getcap( avdt_conn_t *avdt_conn, wiced_bt_avdt_sep_info_t *p_sep_info,
                                          wiced_bt_avdt_cfg_t *avdt_sep_config );

static wiced_result_t av_app_send_discover_req(avdt_conn_t *avdt_conn);
static wiced_result_t av_app_send_start_req(avdt_conn_t *avdt_conn);
static wiced_result_t av_app_disconnect_connection( wiced_bt_device_address_t peer_bda );

static wiced_result_t av_app_reconfigure_req(avdt_conn_t *avdt_conn, uint8_t new_sf, uint8_t new_chcfg );
static wiced_result_t av_app_send_close_req(avdt_conn_t *avdt_conn);
...
static wiced_result_t av_app_send_suspend_req(avdt_conn_t *avdt_conn);
...
wiced_result_t av_app_initiate_sdp(avdt_conn_t *avdt_conn, wiced_bt_device_address_t bda);
static wiced_result_t av_app_stop_audio(void);
#ifdef CYW9BT_AUDIO
static void av_app_am_audio_start(void);
static void av_app_am_audio_stop(void);
#endif

static int8_t avdt_conn_idx_find_idle(void);
static int8_t avdt_conn_idx_by_handle(uint8_t avdt_handle);
static int8_t avdt_conn_idx_by_bda(BD_ADDR bda);
static wiced_bool_t avdt_conn_ready_to_stream(void);
static uint32_t avdt_conn_num(AV_STATE state);
...
uint32_t hci_control_audio_conn_idle_timer_param;
wiced_timer_t hci_control_audio_set_cfg_timer;
uint32_t hci_control_audio_set_cfg_timer_param;
void wiced_audio_start_multi( int is_master, int audio_route, uint16_t *lcid, uint8_t num_lcid, wiced_bt_a2d_sbc_cie_t * pSbc);
void wiced_audio_suspend_multi ( uint16_t lcid[], uint8_t num_lcid, wiced_audio_suspend_complete_cback_t p_cback);
...
#endif // MP3_DECODER_INCLUDED
    int i;
    memset (&av_app_cb, 0, sizeof (tAV_APP_CB));

    wiced_init_timer(&hci_control_audio_conn_idle_timer, av_app_idle_suspend_timeout, (TIMER_PARAM_TYPE) &hci_control_audio_conn_idle_timer_param, WICED_SECONDS_TIMER);
    wiced_init_timer(&hci_control_audio_set_cfg_timer, av_app_send_setconfiguration, (TIMER_PARAM_TYPE) &hci_control_audio_set_cfg_timer_param, WICED_SECONDS_TIMER);
#ifdef MP3_DECODER_INCLUDED
...
uint8_t hci_control_audio_handle_command( uint16_t cmd_opcode, uint8_t* p_data, uint32_t len )
{
    ...
    case HCI_CONTROL_AUDIO_DATA_FORMAT:
        {
            if (avdt_conn_num(AV_STATE_OPEN) == 1)
            {
 ...
 */
static wiced_bool_t av_app_sbc_format_check(avdt_conn_t *avdt_conn, uint8_t *peer_codec_info, wiced_bt_a2d_sbc_cie_t *sbc_caps )
{
    ...
    WICED_BT_TRACE( "[%s]: wiced_bt_a2d_pars_sbc_info status = %d codec type: %d role: %s\n", __FUNCTION__,
                    ...
                    avdt_conn->is_accepter ? "ACCEPTOR" : "INITIATOR");

    /* make sure the endpoint is at least SBC */
    if ( ( a2d_status == A2D_SUCCESS ) && ( peer_codec_info[2] == A2D_MEDIA_CT_SBC ) )
    {
        if (!avdt_conn->is_accepter)
        {
            uint8_t sample_freq = (avdt_conn->reconfigure) ? av_app_cb.reconfig_sf : av_app_cb.audio_sf;
    ...
    if ( ret_val )
    {
        if (!avdt_conn->is_accepter)
        {
            uint8_t ch_config = (avdt_conn->reconfigure) ? av_app_cb.reconfig_chcfg : av_app_cb.audio_chcfg;
...
static void av_app_send_setconfiguration( uint32_t cb_params )
{
    ...
    for ( i = 0; i < av_app_cb.peer_num_seps; i++ )
    {
             ...
             ( av_app_cb.sep_info[i].media_type == av_app_cb.stream_cb.media_type ) )
        {
            ...
            if (peercaps != NULL)
            {
                ...
                if ( av_app_sbc_format_check(avdt_conn, peercaps->codec_info, &av_app_cb.sbc_caps_configured ) )
                {
    ...
    if ( peercaps && (i < av_app_cb.peer_num_seps) )
    {
        ...
        avdt_result = wiced_bt_avdt_open_req( avdt_conn->avdt_handle, avdt_conn->peer_bda, av_app_cb.sep_info[av_app_cb.sep_configured_for_streaming].seid, &av_sbc_cfg );
        if (avdt_result != AVDT_SUCCESS)
        {
            WICED_BT_TRACE( "[%s]: wiced_bt_avdt_open_req() failed. avdt_result=%d", __FUNCTION__, avdt_result );
            av_app_disconnect_connection(avdt_conn->peer_bda);
        }
    }
    else
    {
...
static void av_app_getcap_cmpl( uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data )
{
    ...
    int8_t idx;
    avdt_conn_t *avdt_conn;

    WICED_BT_TRACE("[%s] handle = %d, bd_addr = %B\n", __FUNCTION__, handle, bd_addr);

    /* handle = 0, cannot find by handle */
    if ((idx = avdt_conn_idx_by_bda(bd_addr)) < 0)
    {
        WICED_BT_TRACE( "[%s]: Cannot find av_conn_idx, bd_addr=%B\n", __FUNCTION__, bd_addr);
        return;
    }
    avdt_conn = &av_app_cb.avdt_conn[idx];


    WICED_BT_TRACE( "[%s]: Enter: %s", __FUNCTION__, dump_state_name(avdt_conn->state) );
    ...
    {
        if ( !av_app_next_getcap(avdt_conn, av_app_cb.sep_info, p_data->getcap_cfm.p_cfg ) )
        {
    ...
    WICED_BT_TRACE( "[%s]: getcap complete info... state: %s", __FUNCTION__, dump_state_name(avdt_conn->state) );
    for ( i = 0; i < MAX_SUPPORTED_CODECS; i++ )
    {
    ...
    if (avdt_conn->state == AV_STATE_CONNECTED)
    {
        ...
        hci_control_audio_set_cfg_timer_param = (uint32_t) avdt_conn;
                                        ...
                                        wiced_bt_avdt_cfg_t *avdt_sep_config )
{
    ...
    if ( av_app_cb.sep_info_idx < av_app_cb.peer_num_seps )
    {
        ...
        getcap_status = (*p_req)( avdt_conn->peer_bda,
...
static void av_app_disc_results (uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data)
{
    ...
    /* handle is 0, cannot find by handle */
    if ((idx = avdt_conn_idx_by_bda(bd_addr)) < 0)
    {
        WICED_BT_TRACE( "[%s]: Cannot find avdt_conn_idx, bd_addr=%B\n", __FUNCTION__, bd_addr);
        return;
    }
    ...
    if (sink_cnt != 0)
    {
        ...
        if (avdt_sep_config != NULL)
        {
            ...
            last_getcap = av_app_next_getcap(avdt_conn, p_discover->p_sep_info, avdt_sep_config);
...
static void av_app_connect_event_hdlr(uint8_t handle, BD_ADDR bd_addr)
{
    ...
    /* handle is always 0 here, will be updated on AVDT_OPEN_CFM_EVT */
    WICED_BT_TRACE( "[%s]: Handle: 0x%04x bd_addr=%B\n", __FUNCTION__, handle, bd_addr);
    ...
    av_app_cb.avdt_conn[idx].state = AV_STATE_CONNECTED;
    memcpy(avdt_conn->peer_bda, bd_addr, BD_ADDR_LEN);

    /* Clear current configuration */
    memset(&av_app_cb.sbc_caps_configured, 0, sizeof(av_app_cb.sbc_caps_configured));

    /* Perform SEP discovery */
    av_app_send_discover_req(avdt_conn);
}
...
static void av_app_disconnect_event_hdlr( uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data )
{
    ...
    WICED_BT_TRACE( "[%s]: handle:%d bd_addr=%B\n\r", __FUNCTION__, handle, bd_addr);

    /* handle is always 0, use bd_addr to find avdt_conn */
    if ((idx = avdt_conn_idx_by_bda(bd_addr)) < 0)
    {
        WICED_BT_TRACE( "[%s]: Cannot find avdt_conn_idx, bd_addr=%B\n", __FUNCTION__, bd_addr);
        return;
    ...
    avdt_conn->state = AV_STATE_IDLE;

    if (avdt_conn_num(AV_STATE_IDLE) == AVDT_CONN_MAX)
    {
    ...
    hci_control_audio_send_disconnect_complete( avdt_conn->avdt_handle, p_data->disconnect_ind.err_code, p_data->disconnect_ind.err_param );
...
static void av_app_open_confirm_event_hdlr(uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data)
{
    ...
    if ((idx = avdt_conn_idx_by_handle(handle)) < 0)
    {
        WICED_BT_TRACE( "[%s]: Cannot find avdt_conn_idx, handle=%d\n", __FUNCTION__, handle);
        return;
    }
    avdt_conn = &av_app_cb.avdt_conn[idx];
    ...
    {
        avdt_conn->state = AV_STATE_OPEN;

        WICED_BT_TRACE( "AVDT open confirm success %d\n\r", p_data->open_cfm.lcid);

        avdt_conn->lcid = p_data->open_cfm.lcid;
        avdt_conn->peer_mtu = p_data->open_cfm.peer_mtu;

        /* Set Central Role on watch_dual_a2dp */
        status = hci_control_bt_role_set(bd_addr, HCI_ROLE_CENTRAL);
        if ((status != WICED_BT_SUCCESS) && (status != WICED_BT_PENDING))
        {
            WICED_BT_TRACE( "Set Central Role failed status=%d\n\r", status);
        }

        hci_control_audio_send_connect_complete( bd_addr, AVDT_SUCCESS, avdt_conn->avdt_handle );
        ...
        {
            wiced_bt_avrc_tg_initiate_open(avdt_conn->peer_bda);
        }
#endif
        /* In the midst of a reconfigure restart... */
        if (avdt_conn->reconfigure == WICED_TRUE)
        {
            /* Attempt a start... */
            av_app_start_audio();
        }
    ...
    {
        avdt_conn->reconfigure = WICED_FALSE;
...
static void av_app_start_confirm_event_hdlr( uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data )
{
    ...
    WICED_BT_TRACE( "[%s]: handle: %d, lcid %d err_code: %d", __FUNCTION__, handle, avdt_conn->lcid, p_data->start_cfm.err_code );

    WICED_BT_TRACE("[%s] err_code=%d\n", __FUNCTION__, p_data->start_cfm.err_code);
    if (avdt_conn->state == AV_STATE_STARTING)
    {
        if ( AVDT_SUCCESS == p_data->start_cfm.err_code )
        {
            av_app_cb.is_interrupted = WICED_FALSE;
            avdt_conn->state = AV_STATE_STARTED;
            ...
            if (avdt_conn_ready_to_stream())
            {
                av_app_set_audio_streaming(WICED_TRUE);
            }
            else
            {
                WICED_BT_TRACE("[%s] Waiting for all connection started...\n", __FUNCTION__);
            }
        }
        else
        {
...
static void av_app_start_indication_event_hdlr( uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data )
{
    ...
    WICED_BT_TRACE( "[%s]: handle: %d, lcid %d", __FUNCTION__, handle, avdt_conn->lcid);

    avdt_status = wiced_bt_avdt_start_resp( handle, p_data->start_ind.label, AVDT_SUCCESS );
    WICED_BT_TRACE("[%s] avdt_status=%d\n", __FUNCTION__, avdt_status);

    /* Send the start response */
    if ( !avdt_status )
    {
        avdt_conn->state = AV_STATE_STARTED;
        WICED_BT_TRACE("[%s]is_host_streaming=%d\n", __FUNCTION__, av_app_cb.is_host_streaming);
        ...
        {
            WICED_BT_TRACE("[%s] Starting hci_control_audio_conn_idle_timer...\n", __FUNCTION__);
            hci_control_audio_conn_idle_timer_param = (uint32_t) avdt_conn;
...
static void av_app_suspend_indication_event_hdlr( uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data )
{
    ...
    if ( avdt_conn->state == AV_STATE_STARTED )
    {
...
static void av_app_suspend_confirm_event_hdlr( uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data )
{
    ...
    if (avdt_conn->state == AV_STATE_STARTED)
    {
        ...
        if (avdt_conn->reconfigure)
        {
            av_app_reconfigure_req(avdt_conn, av_app_cb.reconfig_sf, av_app_cb.reconfig_chcfg );
        }
    }
}
...
static void av_app_reconfig_confirm_event_hdlr(uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data)
{
    ...
    WICED_BT_TRACE( "[%s]: <%B> error_code: %d state=%s\n\r", __FUNCTION__, bd_addr, p_data->reconfig_cfm.hdr.err_code, dump_state_name(avdt_conn->state));
    ...
    if (AVDT_SUCCESS == p_data->reconfig_cfm.hdr.err_code)
    {
        ...
        if (avdt_conn->state == AV_STATE_OPEN || av_app_cb.is_interrupted)
        {
            av_app_send_start_req(avdt_conn);
            av_app_cb.is_interrupted = WICED_FALSE;
        }
    }
    else
    {
        ...
        av_app_send_close_req(avdt_conn);
    }
}
...
static void av_app_close_confirm_event_hdlr(uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data)
{
    ...
    if (avdt_conn->reconfigure == WICED_TRUE)
    {
        avdt_conn->state = AV_STATE_CONNECTED; /* We are still connected at the signaling channel level */
...
static void av_app_close_indication_event_hdlr(uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data)
{
    ...
    /* we sent the close request. attempt a disconnect */
    av_app_disconnect_connection(bd_addr);
}
...
static void av_app_config_indication_event_hdlr(uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data)
{
    ...
    if (AVDT_SUCCESS == p_data->config_ind.hdr.err_code)
    {
        ...
        avdt_conn->state = AV_STATE_CONFIGURE;

        /* if we are receiving the config we are the acceptor now */
        avdt_conn->is_accepter = WICED_TRUE;
        ...
        {
            if ( !av_app_sbc_format_check( avdt_conn, p_data->config_ind.p_cfg->codec_info, &sbc_caps ) )
            {
                ...
                avdt_conn->state = AV_STATE_CONNECTED;
            }
        }
        else
        {
...
static void av_app_open_indication_event_hdlr(uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data)
{
    ...
    avdt_conn->lcid = p_data->open_ind.lcid;
    avdt_conn->peer_mtu = p_data->open_ind.peer_mtu;
    ...
    hci_control_audio_send_connect_complete( bd_addr, 0, avdt_conn->avdt_handle );
}
    ...
    case AVDT_CONNECT_IND_EVT:
        ...
        if (event != AVDT_WRITE_CFM_EVT)
        {
            WICED_BT_TRACE ("[%s]: Handle: %d <%B> Event: %d (%s) State: %d (%s)\n\r", __FUNCTION__,
                handle, bd_addr, event, dump_avdt_event_name (event), avdt_conn->state, dump_state_name (avdt_conn->state));
        }
        ...
        if (p_data->connect_ind.err_code == AVDT_SUCCESS)
        {
            ...
            if ( av_app_initiate_sdp(avdt_conn, bd_addr) != WICED_SUCCESS)
            {
                // Peer SDP failed so assume that peer SDP version is 1.2
                av_app_cb.peer_avdt_version = AVDT_VERSION_1_2;
                av_app_connect_event_hdlr( avdt_conn->avdt_handle, avdt_conn->peer_bda);
            }
        }
        break;
...
static wiced_result_t av_app_create_connection(avdt_conn_t *avdt_conn)
{
    uint16_t avdt_status;
    ...
    avdt_status =
            wiced_bt_avdt_connect_req(avdt_conn->peer_bda,
    ...
    avdt_conn->state = AV_STATE_CONNECTING;
...
static wiced_result_t av_app_disconnect_connection(wiced_bt_device_address_t peer_bda)
{
    ...
    if ((idx = avdt_conn_idx_by_bda(peer_bda)) < 0)
    {
        WICED_BT_TRACE( "[%s]: Cannot find avdt_conn_idx, bda=%B\n", __FUNCTION__, peer_bda);
        return status;
    }
    ...
    if ( AVDT_SUCCESS != (status = wiced_bt_avdt_disconnect_req( peer_bda, av_app_disconnect_event_hdlr )) )
    {
        WICED_BT_TRACE( "[%s] failed!! %d\n\r", __FUNCTION__, status );
        status = WICED_ERROR;
    }
    else
    {
        avdt_conn->state = AV_STATE_DISCONNECTING;
...
static wiced_result_t av_app_send_discover_req(avdt_conn_t *avdt_conn)
{
    ...
    WICED_BT_TRACE( "[%s] state: %d\n\r", __FUNCTION__, avdt_conn->state );
    ...
    if (p_sep_info != NULL)
    {
        ...
        max_seps = wiced_bt_get_buffer_size(p_sep_info) / sizeof(wiced_bt_avdt_sep_info_t);
        if ( AVDT_SUCCESS != wiced_bt_avdt_discover_req( avdt_conn->peer_bda, p_sep_info, max_seps, av_app_disc_results ) )
        {
            WICED_BT_TRACE( "[%s] failed!!\n\r", __FUNCTION__ );
            wiced_bt_free_buffer(p_sep_info);
            status = WICED_ERROR;
...
static wiced_result_t av_app_send_start_req(avdt_conn_t *avdt_conn)
{
    ...
    WICED_BT_TRACE( "[%s] avdt_handle=%d\n\r", __FUNCTION__, avdt_conn->avdt_handle);

    if (avdt_conn->state == AV_STATE_OPEN)
    {
        if (avdt_conn->pm_status == WICED_POWER_STATE_SNIFF)
        {
            wiced_bt_dev_cancel_sniff_mode(avdt_conn->peer_bda);
        }
        if ( AVDT_SUCCESS != wiced_bt_avdt_start_req(&avdt_conn->avdt_handle, 1) )
        {
            WICED_BT_TRACE( "[%s] failed!! hdl: %d \n\r", __FUNCTION__, avdt_conn->avdt_handle );
            return WICED_ERROR;
        }

        avdt_conn->state = AV_STATE_STARTING;
    }
...
static wiced_result_t av_app_send_suspend_req(avdt_conn_t *avdt_conn)
{
    ...
    avdt_status = wiced_bt_avdt_suspend_req(&avdt_conn->avdt_handle, 1);
    if (AVDT_SUCCESS != avdt_status)
    {
        WICED_BT_TRACE( "wiced_bt_avdt_suspend_req failed hdl: %d\n\r", avdt_conn->avdt_handle );
        status = WICED_ERROR;
    }
...
static wiced_result_t av_app_send_close_req(avdt_conn_t *avdt_conn)
{
    ...
    WICED_BT_TRACE( "[%s] handle: %d enter state: %s %d\n", __FUNCTION__,
                    avdt_conn->avdt_handle, dump_state_name(avdt_conn->state), avdt_conn->state );
    ...
    avdt_status = wiced_bt_avdt_close_req( avdt_conn->avdt_handle );
    if ( AVDT_SUCCESS != avdt_status )
    {
...
static wiced_result_t av_app_reconfigure_req(avdt_conn_t *avdt_conn, uint8_t new_sf, uint8_t new_chcfg)
{
    ...
    WICED_BT_TRACE( "[%s] enter state: %d seid %d  av_app_cb.sep_info_idx:%d \n", __FUNCTION__,
                    avdt_conn->state ,av_app_cb.sep_info[av_app_cb.sep_configured_for_streaming].seid, av_app_cb.sep_info_idx);

    avdt_conn->reconfigure = WICED_TRUE;
    ...
    if (avdt_conn->state == AV_STATE_STARTED)
    {
         ...
         */
        status = av_app_send_suspend_req(avdt_conn);
    ...
    else
    {
        ...
        /* we will use the first SBC SEP that we find. */
        if ( av_app_sbc_format_check( avdt_conn, peer_caps->codec_info, &sbc_caps ) )
        {
            wiced_bt_avdt_cfg_t av_sbc_cfg;
            ...
            avdt_status = wiced_bt_avdt_reconfig_req( avdt_conn->avdt_handle, &av_sbc_cfg );
            if ( AVDT_SUCCESS != avdt_status )
            {
                avdt_conn->reconfigure = WICED_FALSE;
...
{
    WICED_BT_TRACE( "[%s] \n", __FUNCTION__ );
    avdt_conn_t *avdt_conn = (avdt_conn_t *)(*((uint32_t *) arg));

    /* stop routing data */
    av_app_set_audio_streaming(WICED_FALSE);

    /* Send a suspend if the audio has been paused for more than the timeout. */
    av_app_send_suspend_req(avdt_conn);
}
...
{
    uint8_t num_lcid = 0;
    uint16_t lcid[AVDT_CONN_MAX];
    avdt_conn_t *avdt_conn;
    int i;

    WICED_BT_TRACE( "[%s]: route:%d sample rate:%d\n\r", __FUNCTION__, av_app_cb.audio_route, av_app_cb.audio_sf);

    for (i = 0; i < AVDT_CONN_MAX; i++)
    {
        avdt_conn = &av_app_cb.avdt_conn[i];
        WICED_BT_TRACE("%s:[%d] state=%d av_stream_state=%d lcid=%d\n", __func__, i, avdt_conn->state, av_app_cb.audio_stream_state, avdt_conn->lcid);
        if (avdt_conn->state == AV_STATE_STARTED)
        {
            lcid[num_lcid]  = avdt_conn->lcid;
            num_lcid++;
        }
    }
    ...
    if (num_lcid == 0)
    {
        WICED_BT_TRACE("[%s]: No valid connection to start\n", __FUNCTION__);
        return;
    }

    avdt_conn = av_app_cb.avdt_conn_in_progress;
    ...
    hci_control_audio_send_started_stopped ( avdt_conn->avdt_handle, WICED_TRUE );

    if ((av_app_cb.audio_route == AUDIO_ROUTE_I2S) || (av_app_cb.audio_route == AUDIO_ROUTE_UART))
    {
        /* Use I2S HW interrupt when source is I2S or WAV/MP3 files, I2S HW interrupt timing is every 128 samples. */
        ...
        wiced_audio_start_multi(WICED_TRUE, av_app_cb.audio_route, lcid, num_lcid, &av_app_cb.sbc_caps_configured);
...
{
    WICED_BT_TRACE( "[%s] state: %s %d\n", __FUNCTION__, dump_state_name(av_app_cb.audio_stream_state), av_app_cb.audio_stream_state);

    av_app_cb.audio_stream_state = AV_STREAM_STATE_STOPPED;

    hci_control_audio_send_started_stopped (0, WICED_FALSE );
...
static void av_app_stop_audio_stream( void )
{
    ...
    for (i = 0; i < AVDT_CONN_MAX; i++)
    {
        ...
        {
            lcid[num_lcid]  = avdt_conn->lcid;
            WICED_BT_TRACE("lcid[%d] = %d\n", num_lcid, lcid[num_lcid]);
            num_lcid++;
        }
    }
    ...
    wiced_audio_suspend_multi( lcid, num_lcid, &av_app_audio_stopped_callback );
...
static wiced_result_t av_app_stop_audio(void)
{
    ...
    for (i = 0; i < AVDT_CONN_MAX; i++)
    {
        ...
        WICED_BT_TRACE("[%s][%d] state: %s %d\n", __FUNCTION__, i, dump_state_name(avdt_conn->state), avdt_conn->state );
...
static wiced_result_t av_app_start_audio(void)
{
    ...
    for (i = 0; i < AVDT_CONN_MAX; i++)
    {
        ...
        if (avdt_conn->state == AV_STATE_OPEN)
        {
            status = av_app_send_start_req(avdt_conn);
            if (status != WICED_SUCCESS)
            {
                break;
            }
...
void av_app_sdp_cback( uint16_t sdp_result )
{
    ...
    avdt_conn_t *avdt_conn = av_app_cb.avdt_conn_in_progress;
    ...
    if ( sdp_result == WICED_BT_SDP_SUCCESS )
    {
                      ...
                      UUID_SERVCLASS_AUDIO_SINK, p_rec)) != NULL)
        {
                          ...
                          UUID_PROTOCOL_AVDTP, &elem) == WICED_TRUE)
            {
                ...
                avdt_conn->state = AV_STATE_SDP_DONE;
    ...
    if (avdt_conn->state == AV_STATE_SDP_DONE)
    {
        if (avdt_conn->is_accepter == WICED_FALSE)
        {
            /* Establish L2CAP connection to the AVDTP service */
            status = av_app_create_connection(avdt_conn);
    ...
    if ( WICED_SUCCESS != status )
    {
        ...
        {
             avdt_conn->state = AV_STATE_IDLE;
             hci_control_audio_send_connect_complete( avdt_conn->peer_bda, status, 0 );
        }
        else
        {
            // Peer SDP failed so assume that peer SDP version is 1.2
...
wiced_result_t av_app_initiate_sdp(avdt_conn_t *avdt_conn, wiced_bt_device_address_t bda)
{
    ...
    memcpy(avdt_conn->peer_bda, bda, BD_ADDR_LEN );
    ...
    {
        av_app_cb.avdt_conn_in_progress = avdt_conn;
        if ( wiced_bt_sdp_service_search_attribute_request( avdt_conn->peer_bda, av_app_cb.p_sdp_db, av_app_sdp_cback) )
        {
            avdt_conn->state = AV_STATE_SDP_IN_PROGRESS;
            return WICED_SUCCESS;
        }
        else
        {
...
wiced_result_t a2dp_app_hci_control_connect(uint8_t* p_data, uint32_t len)
{
    ...
    STREAM_TO_UINT8(audio_route, p_data);

    if ((idx = avdt_conn_idx_find_idle()) < 0)
    {
        WICED_BT_TRACE( "[%s]: Cannot find avdt_conn_idx\n", __FUNCTION__);
        return status;
    }
    avdt_conn = &av_app_cb.avdt_conn[idx];

    memcpy(avdt_conn->peer_bda, bd_addr, sizeof(wiced_bt_device_address_t));

    WICED_BT_TRACE( "[%s] %B state: %s \n\r", __FUNCTION__, bd_addr, dump_state_name(avdt_conn->state));

    /* First make sure that there is not already a connection */
    if (avdt_conn->state == AV_STATE_IDLE)
    {
        /* store routing parameter */
        av_app_cb.audio_route = audio_route;
        ...
        status = av_app_initiate_sdp(avdt_conn, bd_addr);
    }
    else
    {
        WICED_BT_TRACE( "[%s]: Bad state: %d", __FUNCTION__, avdt_conn->state);
    }
...
wiced_result_t a2dp_app_hci_control_disconnect( uint8_t* p_data, uint32_t len )
{
    ...
    if ((idx = avdt_conn_idx_by_handle(handle)) < 0)
    {
        WICED_BT_TRACE( "[%s]: Cannot find av_conn_idx, handle=%d\n", __FUNCTION__, handle);
        return status;
    }
    avdt_conn = &av_app_cb.avdt_conn[idx];

    WICED_BT_TRACE( "[%s] state: %s Handle %d\n\r", __FUNCTION__, dump_state_name(avdt_conn->state), handle );
    ...
    if ( avdt_conn->state <= AV_STATE_CONNECTED )
    {
        ...
        status = av_app_disconnect_connection(avdt_conn->peer_bda);
    }
    else
    {
        status = av_app_send_close_req(avdt_conn);
    }
...
{
    uint16_t        handle    = (uint16_t) p_data[0] + ((uint16_t) p_data[1] << 8);
    wiced_result_t  status    = WICED_ERROR;
    ...
    if (av_app_check_configured_settings(&av_app_cb.sbc_caps_configured, new_sf, new_chcfg) == WICED_FALSE)
    {
        for (i = 0; i < AVDT_CONN_MAX; i++)
        {
            WICED_BT_TRACE("%s:%d\n", __func__, i);
            avdt_conn = &av_app_cb.avdt_conn[i];
            if (av_app_cb.avdt_conn[i].state == AV_STATE_OPEN || av_app_cb.avdt_conn[i].state == AV_STATE_STARTED)
            {
                // Need to reconfigure before we start the stream.
                status = av_app_reconfigure_req(avdt_conn, new_sf, new_chcfg);
                if (status != WICED_SUCCESS)
                {
                    break;
                }
...
wiced_result_t a2dp_app_hci_control_stop(uint8_t* p_data, uint32_t len)
{
    ...
    for (i = 0; i < AVDT_CONN_MAX; i++)
    {
        ...
        WICED_BT_TRACE("[%s][%d] state: %s Handle %d\n\r", __FUNCTION__, i, dump_state_name(avdt_conn->state), avdt_conn->avdt_handle);
        ...
        BTM_SetPacketTypes(avdt_conn->peer_bda,
...
static uint16_t a2dp_app_create_sep( void )
{
    ...
    else
    {
        ...
        {
            avdt_conn = &av_app_cb.avdt_conn[i];
            avdt_status = wiced_bt_avdt_create_stream(&avdt_conn->avdt_handle, &av_app_cb.stream_cb);
            if (AVDT_SUCCESS != avdt_status)
            {
...
void avdt_init( )
{
    ...
    for (i = 0; i < AVDT_CONN_MAX; i++)
    {
        av_app_cb.avdt_conn[i].is_accepter = WICED_FALSE;
        av_app_cb.avdt_conn[i].state = AV_STATE_IDLE;
    }
}
...
wiced_result_t av_app_deinit(void)
{
    ...
    for (i = 0; i < AVDT_CONN_MAX; i++)
    {
        if (av_app_cb.avdt_conn[i].state != AV_STATE_IDLE)
        {
        ...
        /* remove AVDT stream */
        wiced_bt_avdt_remove_stream(av_app_cb.avdt_conn[i].avdt_handle);
        av_app_cb.avdt_conn[i].avdt_handle = 0;
...
static void hci_control_audio_mp3_audio_info_update_handler(wiced_bt_mp3_decoder_audio_frame_into_t *p_audio_info)
{
    uint8_t new_sf;
    uint8_t new_chcfg;
    avdt_conn_t *avdt_conn = (avdt_conn_t *) p_audio_info->data;

    WICED_BT_TRACE("hci_control_audio_mp3_audio_info_update_handler (%s) (%d, %s)\n",
                   dump_state_name(avdt_conn->state),
    ...
    av_app_reconfigure_req(avdt_conn, new_sf, new_chcfg);
}
...
static int8_t avdt_conn_idx_find_idle(void)
{
    ...
    for (i = 0; i < AVDT_CONN_MAX; i++)
    {
        //WICED_BT_TRACE("[%s][%d]state = %s (%d)\n", __FUNCTION__, i, dump_state_name(av_app_cb.avdt_conn[i].state), av_app_cb.avdt_conn[i].state);
        if (av_app_cb.avdt_conn[i].state == AV_STATE_IDLE)
        {
            WICED_BT_TRACE("[%s] return idx = %d\n", __FUNCTION__, i);
            return i;
        }
    }

    WICED_BT_TRACE("[%s] Error! Cannot find idle avdt_conn\n", __FUNCTION__);
    return -1;
}
...
static int8_t avdt_conn_idx_by_handle(uint8_t avdt_handle)
{
    ...
    for (i = 0; i < AVDT_CONN_MAX; i++)
    {
        if (av_app_cb.avdt_conn[i].avdt_handle == avdt_handle)
        {
            WICED_BT_TRACE("[%s] return idx = %d\n", __FUNCTION__, i);
            return i;
        }
    ...
    WICED_BT_TRACE("[%s] Error! avdt_handle %d not found\n", __FUNCTION__, avdt_handle);
    return -1;
}
...
static int8_t avdt_conn_idx_by_bda(BD_ADDR bda)
{
    ...
    for (i = 0; i < AVDT_CONN_MAX; i++)
    {
        if (memcmp(av_app_cb.avdt_conn[i].peer_bda, bda, sizeof(BD_ADDR)) == 0)
        {
            WICED_BT_TRACE("[%s] return idx = %d\n", __FUNCTION__, i);
            return i;
        }
    ...
    WICED_BT_TRACE("[%s] Error! bda %B not found\n", __FUNCTION__, bda);
    return -1;
}
...
static wiced_bool_t avdt_conn_ready_to_stream(void)
{
    ...
    for (i = 0; i < AVDT_CONN_MAX; i++)
    {
        //WICED_BT_TRACE("[%d]state = %s\n", i, dump_state_name(av_app_cb.avdt_conn[i].state));
        if (av_app_cb.avdt_conn[i].state == AV_STATE_STARTED || av_app_cb.avdt_conn[i].state == AV_STATE_IDLE)
        {
            cnt++;
        }
    }

    if (cnt == AVDT_CONN_MAX)
    {
        return WICED_TRUE;
    }
...
static uint32_t avdt_conn_num(AV_STATE state)
{
    ...
    for (i = 0; i < AVDT_CONN_MAX; i++)
    {
        if (av_app_cb.avdt_conn[i].state == state)
        {
            cnt++;
        }
    }
